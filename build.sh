...
bison -d -o src/parser.cpp parser.yacc -v # -t for debug
mv src/parser.hpp include/parser.hpp
mv src/parser.output debug/
lex -o src/tokens.cpp tokens.l
g++ -fuse-ld=lld -I include/ -g `llvm-config --cxxflags --ldflags --system-libs --libs all` -lgmp -lntl -DYYDEBUG -o GroMath $FILES